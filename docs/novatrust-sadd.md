...
**Version:** 2.3 — corrects a foundational error: this document previously assumed a custom MVC framework already existed in NovaTrust (`Core/`, `bootstrap/`, `Interfaces/`, etc., all marked "kept"). Verified directly against the live repository: **no such framework exists.** That structure belongs to a separate codebase (Emirates) supplied as a structural reference, not as NovaTrust's own pre-existing code. Section 0, Section 1 (principles 1 and 6), and the entire Section 4 directory tree are corrected below.
...
**v2.3 changelog:** (1) Section 0 rewritten with the verified real repository contents — 14 flat top-level items, zero namespaced/class-based PHP anywhere, no router, no front controller. (2) Guiding Principles 1 and 6 corrected from "extend the existing framework" to "build the framework from scratch, following the Emirates pattern." (3) Section 4's directory tree rewritten in full: every framework-core annotation flips from `(kept)` to `[NEW]`; genuinely existing assets (`assets/css/*.css`, `assets/images/*`, `assets/js/menu.js`, `assets/js/toast.js`) are correctly marked `[EXISTING]`; current flat files are marked `[REPLACES]` where the new structure takes over their role. (4) Section 0.1 corrected: the plaintext DB credentials are duplicated in **both** `config/config.php` and `database/db.php`, not just one file.
...
database/                                [db.php, schema.sql, novatrust.sql, db_update.sql
  ...
  migrations/                            [NEW — see Section 5.4 for the concrete numbered list;
                                          002–009 are baseline snapshots of the real current
                                          schema in schema.sql/novatrust.sql, so the new
...
Full numbered sequence — baseline first (idempotent `CREATE TABLE IF NOT EXISTS`, safe to run against a database that already has these tables live), then the upgrade:

| # | File | Purpose |
|---|---|---|
| 001 | `create_migrations_table.sql` | tracking table, above |
| 002–009 | `create_users_table.sql` … `create_support_requests_table.sql` | baseline snapshot of current schema, one file per existing table |
| 010 | `create_compliance_flags_table.sql` | new |
| 011 | `create_compliance_settings_table.sql` | new |
| 012 | `create_fx_reference_rates_table.sql` | new |
| 013 | `add_flag_id_to_user_compliance_codes.sql` | needs 010 first |
| 014 | `create_virtual_card_requests_table.sql` | new |
| 015 | `add_encrypted_columns_to_virtual_cards.sql` | new |
| 016 | `create_withdrawal_requests_table.sql` | new |
| 017 | `add_withdrawal_request_id_to_transactions.sql` | needs 016 first |
| 018 | `create_chat_conversations_table.sql` | new |
| 019 | `create_chat_messages_table.sql` | needs 018 first |
| 020 | `create_chat_bot_rules_table.sql` | new |
| 021 | `create_mail_settings_table.sql` | new |
| 022 | `add_read_state_to_notifications.sql` | new |
| 023 | `create_audit_log_table.sql` | new |
| 024 | `add_role_to_admins_table.sql` | only if role separation is confirmed |
| 025 | `create_blog_posts_table.sql` | only if the blog goes DB-driven |
