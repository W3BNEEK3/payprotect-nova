...
**Version:** 1.3 — corrects the same foundational error as SADD v2.3: Phase 1 was previously "extensions" to a framework that doesn't exist. Rebuilt as real from-scratch scaffolding (`P1.1`–`P1.11`), original tasks renumbered (`P1.12`–`P1.19`) and re-pointed at the new scaffolding instead of an assumed existing core. `P0.19` corrected to reference the real procedural files, not fictional existing classes.
**Prepared for:** Wynston
**Synthesizes:** `novatrust-business-logic-srs.md` v1.2, `novatrust-sadd.md` v2.3, `novatrust-design-system.md` v1.1
...
**v1.3 changelog:** Verified directly against `github.com/W3BNEEK3/payprotect-nova`: no framework exists in the live repo at all — confirmed by grepping the entire codebase outside `vendor/` for `namespace`/`class` declarations (zero matches) and inspecting the flat `admin/`/`user/` script structure directly. This invalidated v1.2's own Phase 0 framing note and `P0.19`, both of which repeated the false premise. Fixed: (1) Phase 0's framing note now states plainly that this is a from-scratch build, (2) `P0.19` now points at the real procedural files (`database/db.php`, `user/withdraw.php`, `admin/admin_send_money.php`, etc.) instead of nonexistent existing classes, (3) Phase 1 rebuilt in full — `P1.1`–`P1.11` are genuine new scaffolding (autoloading, `EnvLoader`, `Database`, `Router`, `App`, the front controller, base `Model`/`Controller`/`Exceptions`/`Middlewares`, ending in an explicit smoke test), the original eight tasks renumbered `P1.12`–`P1.19` and now depend on that scaffolding existing rather than an assumed framework core, (4) every downstream reference to the old `P1.1`–`P1.8` numbering updated across `P0.6`, `P0.7`, `P2.1`, `P4.1`, `P4.4`, `P4.6`, `P6.1`, `P7.1`, `P8.2`, `P12.1`, `P13.6` — two of which (`P6.1`, `P7.1`) were also re-pointed to the correct prerequisite (the router smoke test, not the API-route task they'd been pointed at).
...
| P0.4 | Decide: hosting target | — | Decision recorded (shared hosting) |
| P0.5 | Decide: fate of `virtual_cards.balance` | — | Decision recorded (formalize it) |
| P0.6 | Decide: admin role separation | — | Decision recorded (keep one flat admin role for now) |
| P0.7 | Check `app/Core/Router.php` | — | Decision recorded (multi-route-file support) |
| P0.8 | Decide: adopt Alpine.js or pure vanilla JS | — | Decision recorded (adopt Alpine.js) |
| P0.9 | Decide: blog implementation | — | Decision recorded (static hardcoded array) |
| P0.10 | Decide: keep `CardIssuerInterface` | — | Decision recorded (skip abstraction, hard-code simulated path) |
| P0.11 | Decide: new-account threshold | — | Decision recorded (30 days) |
| P0.12 | Decide: cumulative vs single-transaction | — | Decision recorded (both) |
| P0.13 | Decide: withdrawal completion model | — | Decision recorded (admin release step) |
| P0.14 | Decide: unclaimed-chat escalation threshold | — | Decision recorded (2 minutes) |
| P0.15 | Decide: Google Fonts/Material Symbols | — | Decision recorded (CDN-linked) |
...
*Implements SADD Section 5, in the exact order Section 5.4 specifies. Every file is written and run against staging first; production only sees a migration after it's been verified there.*

| ID | Task | Depends on | Output |
|---|---|---|---|
| P2.1 | Write and run `001_create_migrations_table.sql` on staging | P1.12 | `migrations` table exists on staging |
| P2.2 | Write and run the baseline snapshot migrations `002`–`009` (idempotent `CREATE TABLE IF NOT EXISTS` for `users`, `admins`, `compliance_requirements`, `transactions`, `virtual_cards`, `notifications`, `refunds`, `support`, `support_requests`) on staging | P2.1 | Baseline schema reproducible from migrations |
| P2.3 | Write and run `010_create_compliance_flags_table.sql`, `011_create_compliance_settings_table.sql`, `012_create_fx_reference_rates_table.sql` on staging | P2.2 | Three new tables |
| P2.4 | Write and run `013_add_flag_id_to_user_compliance_codes.sql` on staging | P2.3 | `user_compliance_codes.flag_id` column + FK |
| P2.5 | Write and run `014_create_virtual_card_requests_table.sql`, `015_add_encrypted_columns_to_virtual_cards.sql` on staging | P2.2 | New table + new encrypted columns (plaintext columns untouched for now) |
| P2.6 | Write and run `016_create_withdrawal_requests_table.sql`, `017_add_withdrawal_request_id_to_transactions.sql` on staging | P2.2 | New table + traceability column |
| P2.7 | Write and run `018_create_chat_conversations_table.sql`, `019_create_chat_messages_table.sql`, `020_create_chat_bot_rules_table.sql` on staging | P2.2 | Three new tables |
| P2.8 | Write and run `021_create_mail_settings_table.sql` on staging | P2.2 | New table |
| P2.9 | Write and run `022_add_read_state_to_notifications.sql`, `023_create_audit_log_table.sql` on staging | P2.2 | Column additions + new table |
| P2.10 | If P0.6 confirmed role separation: write and run `024_add_role_to_admins_table.sql` on staging | P0.6 | `admins.role` column |
| P2.11 | If P0.9 confirmed a DB-driven blog: write and run `025_create_blog_posts_table.sql` on staging | P0.9 | `blog_posts` table |
| P2.12 | Full regression pass on staging: confirm every existing page still works against the migrated schema before touching production | P2.1–P2.11 | Signed-off staging environment |
| P2.13 | **Production migration window:** take a fresh backup (per P0.17's procedure), run `migrate:status`, apply `001`–`025` (as applicable) in order, verify with `migrate:status` again, smoke-test the live site | P2.12, P0.17 | Production schema matches staging |
...
| ID | Task | Depends on | Output |
|---|---|---|---|
| P3.1 | Build core account Models: `User`, `Admin`, `Transaction`, `Refund` | P2.13 | 4 Models |
...
| P3.3 | Build compliance Models: `ComplianceRequirement`, `ComplianceFlag`, `ComplianceSettings`, `FxRate`, `UserComplianceCode` | P2.13 | 5 Models |
...
| P3.5 | Build card & withdrawal Models: `VirtualCard`, `VirtualCardRequest`, `WithdrawalRequest` | P2.13 | 3 Models — `WithdrawalRequest` casts `destination_details` JSON to/from array |
...
| P3.7 | Build notification/chat Models: `Notification`, `ChatConversation`, `ChatMessage`, `ChatBotRule` | P2.13 | 4 Models |
...
| P3.9 | Build remaining Models/Repositories: `MailSetting`/`MailSettingsRepository`, `AuditLog`/`AuditLogRepository`, `Support`, `SupportRequest`, and `BlogPost`/`BlogPostRepository` if P0.9 confirmed | P2.13 | Remaining Models + Repositories |
...
| P4.3 | Write and run the one-time backfill script that encrypts existing plaintext `card_number`/`cvv` values into the new encrypted columns for all existing cards | P2.13, P4.1 | Backfill script, run once against production |
